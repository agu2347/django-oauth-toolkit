...
DEFAULTS = {
    ...
    #
    # Each ``OAUTH_BCP_INSECURE_*_ENABLED`` flag guards a behavior that RFC 9700
    # discourages. ``True`` keeps the insecure/legacy behavior available but emits a
    # warning whenever it is exercised; ``False`` enforces the compliant behavior
    # (the insecure request is rejected, or the secure behavior is performed).
    #
    # They all default to ``True`` so upgrading does not change runtime behavior.
    ...
    "OAUTH_BCP_INSECURE_IMPLICIT_GRANT_ENABLED": True,
    "OAUTH_BCP_INSECURE_PASSWORD_GRANT_ENABLED": True,
    "OAUTH_BCP_INSECURE_PKCE_PLAIN_ENABLED": True,
